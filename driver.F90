...
! !LOCAL VARIABLES:
  ...
  character(len=256) :: pritchsoilfile
  character(len=256) :: iam_str
!  integer :: irec, reclen, irow, icol
  character(len=256) :: HCtemperaturefileTG
  character(len=256) :: HCtemperaturefileTSA
...
if (nstep .eq. 0) then
     write (iam_str, "(I3.3)") iam
     pritchsoilfile = trim('pritch_soil_')//trim(iam_str)
    !INQUIRE(iolength=reclen) cptr%cws%h2osoi_liq
    !print*,"iolength=",reclen 
#ifdef pritchsoilwrite
    open(555,STATUS='NEW',FILE=pritchsoilfile)
    !open(555,STATUS='NEW',FILE=pritchsoilfile,access='direct',recl=reclen,form='binary')
#endif 
#ifdef pritchsoilread    
    open(555,STATUS='OLD',FILE='/scratch/04080/hongcheq/GLACE/SPCAM3.5/SPCAM3.5_yes_crm1994/spcam3.5_yescrm_SM_write/'//pritchsoilfile)
    !open(555,STATUS='OLD',FILE='/scratch/04080/hongcheq/GLACE/SPCAM3.5/SPCAM3.5_no_crm1994/spcam3.5_nocrm_SM_write/'//pritchsoilfile)
    !open(555,STATUS='OLD',FILE=pritchsoilfile,access='direct',recl=reclen,form='binary')
!To avoid copying around
    !open(555,STATUS='OLD',FILE='/scratch/04080/hongcheq/GLACE/SPCAM3.5/SPCAM3.5_no_crm1994/spcam3.5_nocrm_SM_write/'//pritchsoilfile,access='direct',recl=reclen,form='binary')
#endif 
end if

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Preparation of ground temperature and 2m air temperature writing/reading
if (nstep .eq. 0) then
     write (iam_str, "(I3.3)") iam
     HCtemperaturefileTG = trim('HC_TG_')//trim(iam_str)
     HCtemperaturefileTSA = trim('HC_TSA_')//trim(iam_str)
#ifdef HCtemperaturewrite
    open(666,STATUS='NEW',FILE=HCtemperaturefileTG)
    open(777,STATUS='NEW',FILE=HCtemperaturefileTSA)
#endif 
#ifdef HCtemperatureread    
    open(666,STATUS='OLD',FILE='/work/04080/hongcheq/SPCAM3.5/BowenRatio_test_TG_TSA_prescription/SPCAM3.5_no_crm/'//HCtemperaturefileTG)
    open(777,STATUS='OLD',FILE='/work/04080/hongcheq/SPCAM3.5/BowenRatio_test_TG_TSA_prescription/SPCAM3.5_no_crm/'//HCtemperaturefileTSA)
#endif 
end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!CSD$ PARALLEL DO PRIVATE (nc,begg,endg,begl,endl,begc,endc,begp,endp)
  ...
  do nc = 1,nclumps
     ...
     !====Hongchen: Better to move this GLACE modification after hydro2,test,09/01/2016===
!#ifdef pritchsoilwrite
!08/30/2016, Hongchen: only subsurface (deeper than 5cm) soil moisture are prescribed
!    irec = 1 + nstep * size(cptr%cws%h2osoi_liq)
!    do irow = 1,size(cptr%cws%h2osoi_liq(:,1)),1   
!        do icol = -4,10,1 
!        write(555,rec=irec) cptr%cws%h2osoi_liq(irow,icol)
!        irec = irec + 1
!        end do
!    end do
!#endif
...
!    do irow = 1,size(cptr%cws%h2osoi_liq(:,1)),1   
!        do icol = -4,10,1   
!        read(555,rec=irec) cptr%cws%h2osoi_liq(irow,icol)
!        irec = irec + 1
!        end do
!    end do
!#endif
!==============================================
#ifdef pritchsoilwrite
          write (555,*) cptr%cws%h2osoi_liq(:,3:10)   
!only subsurface (deeper than 5cm) soil moisture are prescribed 
#endif
#ifdef pritchsoilread
          read (555,*) cptr%cws%h2osoi_liq(:,3:10)
#endif

!!! TG, TSA writing/reading capabilities
#ifdef HCtemperaturewrite
          write (666,*) cptr%ces%t_grnd(:)   
          write (777,*) cptr%p%pes%t_ref2m(:)  
#endif
#ifdef HCtemperatureread
          read (666,*) cptr%ces%t_grnd(:)
          read (777,*) cptr%p%pes%t_ref2m(:)
#endif
